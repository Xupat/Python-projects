...
class Button():
	def __init__(self, rect, text, font_size, font_color='white', font_name=None, press_color=(50, 150, 150), color=(100, 100, 100), border=0, border_radius=4, border_color=(0, 0, 0)):
		...
		self.press=False
	def draw(self, surf):
		text=self.gui_font.render(str(self.text), True, self.font_color)
...
class Switch_Button():
	def __init__(self, rect, text,press_text, font_size, font_color='white', font_name=None, press_color=(50, 150, 150), color=(100, 100, 100), change_color=True, border=0, border_radius=4, border_color=(0, 0, 0)):
		self.rect=pygame.Rect(rect)
		
		self.gui_font=pygame.font.Font(font_name, font_size)
		self.off_text=text
		self.on_text=press_text
		self.font_color=font_color
		self.now_text=self.off_text
	...
	def draw(self, surf):
		...
		text=self.gui_font.render(str(self.now_text), True, self.font_color)
		text_rect=text.get_rect(center=self.rect.center)
	...
	def touch(self):
		for event in pygame.event.get():
			if event.type==pygame.MOUSEBUTTONDOWN and self.rect.collidepoint(pygame.mouse.get_pos()):
				...
				self.now_text=self.off_text if self.now_text!=self.off_text else self.on_text
				return self.press
...
class Slider():
	def __init__(self, point1, point2, min_val, max_val,step_size, start_val, stable='y', line_width=10):
		self.point1=point1
		self.point2=point2
		self.stable=stable
		...
		if self.stable=='y':
			self.x=self.point1[0]+self.segmant*(start_val-self.min)
			self.y=self.point1[1]
		else:
			self.x=self.point1[0]
			self.y=self.point1[1]+self.segmant*(start_val-self.min)
		...
		if pygame.mouse.get_pressed()[0] and self.hold:
			if self.stable=='y':
				if mx >self.point2[0]:
					...
					self.value=self.max
				elif mx<self.point1[0]:
					self.x=self.point1[0]
					self.value=self.min
				elif self.point1[0] < mx <self.point2[0]:
					self.x=mx
