...
def minimax(bored, alpha, beta, isMax, depth):
...
def main():
	...
	while run:
		...
		for e in events:
			if e.type==pygame.MOUSEBUTTONDOWN and result:
				reset()
				
		if ai==turn and not result and stop==0:
			i, j=next_move(bored)
			bored[i][j]=turn
			turn=other(turn)
			stop=40
		if player==turn and not result and stop==0:
			i, j=next_move(bored)
			bored[i][j]=turn
			turn=other(turn)
			stop=40
